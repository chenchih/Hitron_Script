...
proc run_case1 {} {
	...
	if {[catch {set ::txt [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.4.1.35604.2.2.1.0]} msg]} {
		SeeEnd "$msg\n"		
	
		} else {
				SeeEnd "current frequency : $::txt\n"
				
				}
	
	if {[catch {set ::txt [snmp_get -v 2c -c private $IP .1.3.6.1.2.1.2.2.1.6.2]} msg] 	} {   #ifPhysAddress 
		SeeEnd "$msg\n"
		
		} else {
				SeeEnd "ifPhysAddress : $::txt\n"
				
				#.txt insert end $msg    ms==>.iso.3.6.1.2.1.2.2.1.6.2 =  Hex: 5C 35 3B 7D FD BB 
				}
