...
#	set upgrade_SW [.ent3 get]
	...
	.txt insert end "insert frequency12: [snmp_set  -v 2c -c private $IP .1.3.6.1.4.1.35604.1.19.3.1.1.0 i "$tftp_ip"]\n"
												
.txt insert end "---------------------------------------------------------------------------\n"
 .txt insert end "insert autoconfigure:  [snmp_set -v 2c -c private $IP  .1.3.6.1.4.1.35604.1.19.52.1.1.204.1.5.2.0 i 1 ]\n"
 .txt insert end "---------------------------------------------------------------------------\n"
  .txt insert end "apply autoconfigure:  [snmp_set -v 2c -c private $IP  .1.3.6.1.4.1.35604.1.19.52.1.1.204.1.6.0 i 1 ]\n"
  .txt insert end "---------------------------------------------------------------------------\n"
  .txt insert end "dns proxy:  [snmp_set -v 2c -c private $IP  .1.3.6.1.4.1.35604.1.19.52.1.1003.1.2.0 i 1 ]\n"
    .txt insert end "---------------------------------------------------------------------------\n"
...
proc run_case1 {} {
	...
	set str [.ent2 get]
	.txt delete 0.0 end
	.txt insert end "current frequency: [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.4.1.35604.1.19.3.1.1.0]\n"
...
#set ::txt4  [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.4.1.35604.2.2.1.0]
	...
	#if {[catch {::txt4  [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.4.1.35604.1.19.52.1.1.204.1.5.2.0]} 1]} {
...
set autoconfigure [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.4.1.35604.1.19.52.1.1.204.1.5.2.0]	
if {$autoconfigure== 0} {
			set ::txt4  "STATELESS"
		} else {
				set ::txt4  "STATEFUL"
...
#set ::txt7  [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.2.1.17.4.3.1.1.92.53.59.166.4.20]
	if {[catch {set temp [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.2.1.2.2.1.6.20]} re]} {
		 ...
		 #etho mac
		if {[catch {set temp [snmp_get -t 5 -Oqv $IP private .1.3.6.1.2.1.2.2.1.6.1]} re]} {
	
			if {
				regexp {([\w]+\s[\w]+\s[\w]+\s[\w]+\s[\w]+\s[\w]+)} $temp match ::txt8 
				set ::txt8[string tolower [ regsub -all {\s} $::txt9 {:}]] 
			}
			...
			set ::txt8  [string toupper [ regsub -all {\s} $::txt8  {:}]] 
			 ...
			 #ipv6 wan
		if {[catch {set temp [snmp_get -t 5 -Oqv $IP private .1.3.6.1.4.1.35604.1.19.52.1.1.204.2.1.1.2.2]} re]} {
		...
		regexp {([\w]+\s[\w]+\s[\w]+\s[\w]+\s[\w]+\s[\w]+)} $temp match ::txt9 
			...
			#regexp {([\w+\s+\w+\s]+)} $temp match ::txt9 
			regexp {([\w+\s]+)} $temp match ::txt9 
...
#if {[catch {::txt10  [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.4.1.35604.1.19.52.1.1003.1.2.0]} 1]} {
#														
#		set ::txt10 "Disable"
#		} else {
#				set ::txt10 "Enable"}	
set dnsproxy [snmp_get -t 5 -Oqv $IP private  .1.3.6.1.4.1.35604.1.19.52.1.1003.1.2.0]	
...
label .lab5 -text "FREQ EX: 418000000" -font [font create -family "Times bold" -size 12  ] -foreground red
...
label .lab9 -text "cm ipv6" -font [font create -family "Times" -size 12]
label .lab10 -text "Dns proxy" -font [font create -family "Times" -size 12]
...
ttk::button .but2 -text "SET" -command "run_case" -default "active"
...
#dns proxy
place .ent8 -in .frm -x 380 -y 290 -width 180 -height 30 
